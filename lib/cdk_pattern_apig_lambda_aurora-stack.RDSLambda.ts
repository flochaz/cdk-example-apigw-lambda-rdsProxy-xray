...
exports.handler = async function (event: any) {
  ...
  await new Promise((resolve, reject) => {
    const queryString = 'CREATE DATABASE IF NOT EXISTS cdkpatterns';
  ...
  let queryResult = await new Promise((resolve, reject) => {
    const queryString = `SELECT * FROM rds_proxy`;
    ...
    connection.query(`SELECT * FROM rds_proxy`, function (error: any, results: any, fields: any) {
      ...
      resolve(
        'All Current Data in rds_proxy Table (url is whatever url you hit on the HTTP API, try another random url like /hello) ' +
          tableString
      );
    });
    // Retrieve the most recently created subsegment
